...
with tab3:
    display = profitable_raw.rename(columns={
        ...
        "avg_holding_days": "Avg Holding Days", "sharpe_ratio": "Sharpe Ratio"
    })
...
if selected_pair in all_trades:
    ...
    fig2.update_layout(
        title="Z-score with Raw Signals",
    ...
    fig3.update_layout(
        title="Cumulative Return — Raw vs LSTM",
    ...
    with tl1:
        st.dataframe(format_trades(raw_trades),
            use_container_width=True, hide_index=True)
    with tl2:
        st.dataframe(format_trades(lstm_trades),
            use_container_width=True, hide_index=True)